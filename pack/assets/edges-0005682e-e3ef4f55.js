import { r as evaluate, c as getConfig$1, d as sanitizeText$2, ao as decodeEntities, h as select, l as log$1, z as utils, ap as curveBasis } from "./mermaid.core-52070432.js";
import { c as createText } from "./createText-3b1f58a4-bc8242e1.js";
import { p as parseMember } from "./svgDraw-70101091-9da7a13f.js";
...
const labelHelper = async (parent, node, _classes, isNode) => {
  ...
  if (node.labelType === "markdown") {
    ...
    text = textNode.appendChild(createLabel$1(sanitizeText$2(decodeEntities(labelText), getConfig$1()), node.labelStyle, false, isNode));
  }
...
const rectWithTitle = (parent, node) => {
  ...
  const halfPadding = node.padding / 2;
  select(descr).attr("transform", "translate( " + // (titleBox.width - bbox.width) / 2 +
  (bbox.width > titleBox.width ? 0 : (titleBox.width - bbox.width) / 2) + ", " + (titleBox.height + halfPadding + 5) + ")");
  select(text).attr("transform", "translate( " + // (titleBox.width - bbox.width) / 2 +
...
const positionEdgeLabel = (edge, paths) => {
  ...
  if (edge.label) {
    ...
    if (path) {
      ...
      log$1.info("Moving label " + edge.label + " from (", x, ",", y, ") to (", pos.x, ",", pos.y, ") abc78");
...
const intersection = (node, outsidePoint, insidePoint) => {
  ...
  if (Math.abs(y - outsidePoint.y) * w > Math.abs(x - outsidePoint.x) * h) {
    ...
    log$1.warn(`sides calc abc89, Q ${Q}, q ${q}, R ${R}, r ${r}`, {
      _x,
      _y
    });
    if (r === 0) {
...
const insertEdge = function(elem, e, edge, clusterDb, diagramType, graph) {
  ...
  if (head.intersect && tail.intersect) {
    ...
    log$1.info("Last point", points[points.length - 1], head, head.intersect(points[points.length - 1]));

import { s as setupDevtoolsPlugin, w as watch } from "./app-87b15710.js";
...
var setupDevtools = (app, globalComputed) => {
  setupDevtoolsPlugin({
    ...
    api.on.inspectComponent((payload) => {
      payload.instanceData.state.push(...globalComputedEntries.map(([name, item]) => ({
    ...
    api.on.getInspectorState((payload) => {
      ...
      if (payload.nodeId === INSPECTOR_GLOBAL_COMPUTED_ID) {
        payload.state = {
          [INSPECTOR_GLOBAL_COMPUTED_LABEL]: globalComputedEntries.map(([name, item]) => ({
            key: name,
            value: item.value
          }))
        };
